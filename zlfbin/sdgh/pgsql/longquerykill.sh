...
# 	-c "SELECT procpid,current_query FROM pg_stat_activity where  and current_query <> '<IDLE>' order by query_start;" \
#	| awk '{for (i=1;i<=NF;i++) {if (($i ~ /^[0-9]/) && ($i ~ /[0-9]$/)) printf("SELECT pg_cancel_backend(%s); /*%s*/\n",$i,$0)}}' \
#   	| tee /data/share/longquerykill.t
psql -U postgres -h $host -p $port \
 	-c "SELECT procpid,current_query FROM pg_stat_activity where query_start < (now()-time'00:03:00') and current_query <> '<IDLE>' order by query_start;" \
